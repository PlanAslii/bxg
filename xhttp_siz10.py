...
FINGERPRINTS = {
    "chrome": {"content-type": "application/grpc", "cache-control": "no-cache, no-store", "x-accel-buffering": "no"},
    "plain": {"content-type": "application/octet-stream", "cache-control": "no-store", "x-accel-buffering": "no"},
}
DEFAULT_FINGERPRINT = "chrome"

def _resp_headers(fp: str) -> dict: return dict(FINGERPRINTS.get(fp, FINGERPRINTS[DEFAULT_FINGERPRINT]))
...
class _AdaptiveFlow:
    ...
    def should_drain(self, buf_size: int) -> bool: return buf_size > self.high_water
    ...
    async def drain(self, writer: asyncio.StreamWriter):
        ...
        if elapsed_ms < FLOW_FAST_DRAIN_MS: self.high_water = min(FLOW_MAX_HW, int(self.high_water * 1.5) + 65536)
        elif elapsed_ms > FLOW_SLOW_DRAIN_MS: self.high_water = max(FLOW_MIN_HW, self.high_water // 2)

def _req_client_ip(request: Request) -> str:
    for header in ["cf-connecting-ip", "true-client-ip", "fly-client-ip", "x-real-ip", "x-forwarded-for"]:
...
async def _check_link(uuid: str):
    ...
    if not is_link_allowed(link): raise HTTPException(status_code=403, detail="not authorized")
...
async def _get_or_create_session(uuid: str, mode: str, session_id: str, ip: str = "نامشخص") -> dict:
    async with XHTTP_LOCK:
        ...
        connections[conn_id] = {
            "uuid": uuid, "ip": ip, "connected_at": datetime.now().isoformat(), "bytes": 0, "transport": f"xhttp-{mode}",
        }
        sess = {
            "uuid": uuid, "mode": mode, "writer": None, "downlink_task": None, "uplink_task": None,
            "down_q": asyncio.Queue(maxsize=DOWNLINK_QUEUE_MAX), "last_seen": time.time(),
            "conn_id": conn_id, "tcp_open": False, "closed": False, "seq_buf": {}, "next_seq": 0,
            "gate": None, "flow": None,
        }
    ...
    while True:
        ...
        async with XHTTP_LOCK:
            ...
            for sid, s in xhttp_sessions.items():
                ...
                if s.get("tcp_open") and idle > SESSION_IDLE_TIMEOUT_ACTIVE: stale.append(sid)
                elif not s.get("tcp_open") and idle > SESSION_IDLE_TIMEOUT: stale.append(sid)
        for sid in stale: await _teardown(sid)
...
async def xhttp_downlink(mode: str, uuid: str, session_id: str, request: Request):
    ensure_reaper()
    if mode not in ("packet-up", "stream-up"): raise HTTPException(status_code=404, detail="unknown mode")
    ...
    try:
        ...
        if seq == sess["next_seq"]:
            ...
            while sess["next_seq"] in sess["seq_buf"]:
                ...
                sess["next_seq"] += 1
        else: sess["seq_buf"][seq] = body
        if sess["writer"].transport.get_write_buffer_size() > PACKET_UP_HIGH_WATER: await sess["writer"].drain()
...
async def stream_up_upload(uuid: str, session_id: str, request: Request):
    ...
    if sess.get("closed"): raise HTTPException(status_code=404, detail="session closed")
    gate = sess.get("gate") or _QuotaGate(uuid); sess["gate"] = gate
    flow = sess.get("flow") or _AdaptiveFlow(); sess["flow"] = flow
    ...
    try:
        async for chunk in request.stream():
            ...
            sess["last_seen"] = time.time()
            if not await gate.add(len(chunk)): raise HTTPException(status_code=403, detail="quota/disabled")
            stats["total_requests"] += 1
            ...
            if writer is None:
                ...
                continue
            writer.write(chunk)
            if flow.should_drain(writer.transport.get_write_buffer_size()): await flow.drain(writer)
    except HTTPException:
        await gate.flush(); await _teardown(session_id); raise
    except Exception as exc:
        error_logs.append({"error": str(exc), "time": datetime.now().isoformat()})
        await gate.flush(); await _teardown(session_id)
